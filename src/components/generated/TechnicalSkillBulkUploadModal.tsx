...
interface TechnicalSkillBulkUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImportComplete: () => void;
}

const TechnicalSkillBulkUploadModal: React.FC<TechnicalSkillBulkUploadModalProps> = ({
  isOpen,
  onClose,
  onImportComplete,
}) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        jsonData.forEach((row: any, index: number) => {
          ...
          const skill = row[1]?.toString()?.trim();

          if (!category || !skill) {
            invalid.push(`Zeile ${index + 1}: Kategorie="${category || 'leer'}", Skill="${skill || 'leer'}"`);
            return;
          }

          parsed.push({ category, skill });
        });
        ...
        parsed.forEach(item => {
          const key = `${item.category}:${item.skill}`;
          if (!seen.has(key)) {
            seen.add(key);
  ...
  const executeImport = async () => {
    ...
    try {
      const response = await fetch('/api/technical-skills/bulk-import', {
        method: 'POST',
        headers: {
            ...
            <header className="flex items-center justify-between p-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50">
              <div>
                <h1 className="text-xl font-semibold text-gray-900">
                  Technical Skills Import
                </h1>
                <p className="text-sm text-gray-600 mt-1">
                  Excel-Import für Technical Skills und Kategorien
                </p>
              </div>
              <button
                onClick={handleClose}
                ...
                <div className="space-y-6">
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 mb-6">
                      Laden Sie eine Excel-Datei mit Technical Skills und Kategorien hoch
                    </p>
                  </div>

                  {/* File Format Info */}
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <h4 className="font-medium text-blue-900 mb-2">Dateiformat:</h4>
                    <ul className="text-sm text-blue-800 space-y-1">
                      <li>• <strong>Spalte A:</strong> Kategorie (z.B. "Allgemeine Kenntnisse")</li>
                      <li>• <strong>Spalte B:</strong> Skill-Name (z.B. "Clean Code")</li>
                      <li>• Erste Zeile wird als Header ignoriert</li>
                      <li>• Leere Zeilen werden ignoriert</li>
                      <li>• Duplikate innerhalb einer Kategorie werden ignoriert</li>
                    </ul>
                  </div>
              ...
              {step === 'importing' && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">
                    Bitte warten Sie, während die Skills und Kategorien importiert werden.
                  </p>
                </div>
              )}
              ...
              {step === 'results' && importResults && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-blue-600">{importResults.skillsCreated}</div>
                      <div className="text-sm text-blue-800">Neue Skills</div>
                    </div>
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-gray-600">{importResults.skillsIgnored}</div>
...
export default TechnicalSkillBulkUploadModal;
