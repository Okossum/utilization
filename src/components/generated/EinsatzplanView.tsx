...
export function EinsatzplanView() {
  ...
  // Lade Daten aus der Einsatzplan-Collection
  const loadEinsatzplanData = async () => {
    if (!user) return;
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = einsatzplanData.filter(item => {
      // Suchfilter
    ...
    return filtered;
  }, [einsatzplanData, searchTerm, filterCC, filterLBS, filterTeam, sortBy, sortDirection]);
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `einsatzplan-${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div className="w-full flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Alle Daten aus der Einsatzplan-Collection
            </p>
          </div>
      ...
      <main className="w-full p-4 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-2xl font-bold text-blue-600">{einsatzplanData.length}</div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900">
              Einsatzplan Daten ({filteredAndSortedData.length} von {einsatzplanData.length})
            </h3>
          </div>
          ...
          {filteredAndSortedData.length === 0 && (
            <div className="p-8 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {einsatzplanData.length === 0 ? 'Keine Daten verfügbar' : 'Keine Daten gefunden'}
              </h3>
              <p className="text-gray-600">
                {einsatzplanData.length === 0 
                  ? 'Lade Daten aus der Einsatzplan-Collection oder prüfe die Verbindung zur Datenbank.'
